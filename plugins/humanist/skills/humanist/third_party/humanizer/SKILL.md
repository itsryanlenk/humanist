# VENDORED REFERENCE - NOT AN INSTALLABLE SKILL

This is stage 1 of the humanist pipeline, vendored from
[blader/humanizer](https://github.com/blader/humanizer) v2.8.0 by Siqi Chen (MIT).
The parent skill reads it and applies it. Read `../../SKILL.md` for how it fits.

**Its YAML frontmatter has been deliberately removed, and that is the only change
to this file.** Upstream ships live frontmatter, which means a plugin loader
registers this as a second, independent skill with `Write` and `Edit` in its
allowed-tools. That skill is a rewrite pass with no gate behind it, competing for
the parent's own trigger phrases - and a rewrite pass that grades its own output is
the precise configuration this project exists to argue against. Removing the
frontmatter makes the file documentation, which is what it is used as here.

**Licensing, which is not simply MIT.** The pattern catalog below is substantially
derived from Wikipedia's
[Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing)
(maintained by WikiProject AI Cleanup), and upstream's own changelog for v2.0.0
says so: _"Complete rewrite based on raw Wikipedia article content."_ Measured, 618
words across 51 maximal verbatim runs - 27.5% of the operative catalog - are
Wikipedia text. Wikipedia is [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/),
whose ShareAlike condition MIT cannot satisfy, so **that material is not available
to you under this repository's MIT grant.** Siqi Chen's `LICENSE` sits beside this
file and governs the original portions. See `../../../../../../docs/attribution.md`
for the full inventory and the measurement.
